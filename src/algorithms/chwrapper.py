"""
CHWrapper (CH + Dijkstra Combination) Routing System

This module uses fixed-weight routing for CHWrapper baseline.
Uses fixed weights: 0.55 distance + 0.45 time (slightly distance-favored).

Key Characteristics:
    - Fixed weights: 0.55 distance, 0.45 time
    - Simulates hybrid approach behavior
    - Baseline comparison algorithm
    - Optimal: Yes (for fixed metric)
...
def create_route_chwrapper(
    net: sumolib.net.Net, start_edge: str, goal_edge: str, max_iterations: int = 1000
) -> List[str]:
    """
    Create route using fixed weights (0.55 distance + 0.45 time).
    ...
    """
    DISTANCE_WEIGHT = 0.55  # Slightly distance-favored
    TIME_WEIGHT = 0.45
    ...
    while current_edge != goal_edge and iterations < max_iterations:
        ...
        for edge in edges_leaving:
            try:
                ...
                # Fixed cost: 0.55 * distance + 0.45 * time
...
def run_simulation(
    ...
    """Run simulation with CHWrapper routing."""
    ...
    # Build SUMO command with CHWrapper routing
    ...
    sumo_cmd.extend(
        ["--quit-on-end", "--routing-algorithm", "CHWrapper"]  # CH + Dijkstra hybrid
    )
    ...
    try:
        print("=" * 80)
        print("CHWRAPPER (CH + DIJKSTRA) ROUTING")
        print("=" * 80)
        print(
            f"Algorithm: CHWrapper (SUMO built-in with --routing-algorithm CHWrapper)"
        )
        ...
        # Allow traffic to load first with corrected wait times
        ...
        # Spawn vehicle and let SUMO compute route with built-in CHWrapper
        vehicle_id = "ambulance"
        print(f"\nValidating edge reachability...")
        try:
            traci.route.add("chwrapper_route", [start_edge])
            traci.vehicle.add(
                vehicle_id, "chwrapper_route", typeID="emergency", depart="now"
            )
            traci.vehicle.setColor(vehicle_id, (255, 0, 255, 255))

            # Set max speed to 80 km/h
            traci.vehicle.setMaxSpeed(vehicle_id, 80.0 / 3.6)
        ...
        print("\n" + "=" * 80)
        if arrival_detected:
            print("[OK] DESTINATION REACHED (CHWrapper)")
            print("=" * 80)
        ...
        print(f"Algorithm: CHWrapper (Hybrid)")
        print("=" * 80)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="CHWrapper (CH + Dijkstra) Routing",
