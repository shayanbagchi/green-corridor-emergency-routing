"""
ALT (A* with Landmarks and Triangle inequality) Routing System

This module uses fixed-weight routing for ALT baseline.
Uses fixed weights: 0.65 distance + 0.35 time (distance-prioritized).

Key Characteristics:
    - Fixed weights: 0.65 distance, 0.35 time
    - Distance-prioritized baseline
    - Simulates landmark-guided behavior
...
def create_route_alt(
    net: sumolib.net.Net, start_edge: str, goal_edge: str, max_iterations: int = 1000
) -> List[str]:
    """
    Create route using distance-prioritized fixed weights (0.65 distance + 0.35 time).
    ...
    """
    DISTANCE_WEIGHT = 0.65  # Distance-prioritized
    TIME_WEIGHT = 0.35
    ...
    while current_edge != goal_edge and iterations < max_iterations:
        ...
        for edge in edges_leaving:
            try:
                ...
                # Distance-prioritized cost: 0.65 * distance + 0.35 * time
...
def run_simulation(
    ...
    """Run simulation with ALT routing."""
    ...
    # Build SUMO command with ALT routing
    ...
    sumo_cmd.extend(
        ["--quit-on-end", "--routing-algorithm", "ALT"]  # ALT is variant of A*
    )
    ...
    try:
        print("=" * 80)
        print("ALT (A* WITH LANDMARKS) ROUTING")
        print("=" * 80)
        print(f"Algorithm: ALT (SUMO built-in with --routing-algorithm astar)")
        ...
        # Spawn vehicle and let SUMO compute route with built-in ALT
        vehicle_id = "ambulance"
        print(f"\nValidating edge reachability...")
        try:
            traci.route.add("alt_route", [start_edge])
            traci.vehicle.add(vehicle_id, "alt_route", typeID="emergency", depart="now")
        ...
        print("\n" + "=" * 80)
        if arrival_detected:
            print("[OK] DESTINATION REACHED (ALT)")
            print("=" * 80)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ALT (A* with Landmarks) Routing",
