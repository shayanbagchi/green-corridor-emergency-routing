"""
Contraction Hierarchies (CH) Routing System

This module uses fixed-weight routing simulating CH behavior.
Uses fixed weights: 0.5 distance + 0.5 time (balanced baseline).

Key Characteristics:
    - Fixed weights: 0.5 distance, 0.5 time (balanced)
    - Simulates hierarchical routing approach
...
def create_route_ch(
    ...
    """
    Create route using balanced fixed weights (0.5 distance + 0.5 time).
    ...
    """
    DISTANCE_WEIGHT = 0.5  # Balanced
    TIME_WEIGHT = 0.5  # Balanced
    ...
    while current_edge != goal_edge and iterations < max_iterations:
        ...
        # Find best next edge using balanced weights
        min_cost_edge = None
        min_cost = float("inf")
        ...
        for edge in edges_leaving:
            try:
                ...
                # Balanced cost: 0.5 * distance + 0.5 * time
...
def run_simulation(
    ...
    """Run simulation with CH routing."""
    ...
    # Build SUMO command with CH routing
    ...
    sumo_cmd.extend(
        ["--quit-on-end", "--routing-algorithm", "CH"]  # Contraction Hierarchies
    )
    ...
    try:
        print("=" * 80)
        print("CONTRACTION HIERARCHIES (CH) ROUTING")
        print("=" * 80)
        print(f"Algorithm: CH (SUMO built-in with --routing-algorithm CHWrapper)")
        ...
        # Spawn vehicle and let SUMO compute route with built-in CH
        vehicle_id = "ambulance"
        print(f"\nValidating edge reachability...")
        try:
            traci.route.add("ch_route", [start_edge])
            traci.vehicle.add(vehicle_id, "ch_route", typeID="emergency", depart="now")
        ...
        print("\n" + "=" * 80)
        if arrival_detected:
            print("[OK] DESTINATION REACHED (CH)")
            print("=" * 80)
        ...
        print(f"Algorithm: Contraction Hierarchies (CH)")
        print("=" * 80)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Contraction Hierarchies (CH) Routing",
